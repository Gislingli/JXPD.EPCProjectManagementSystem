...
class Details extends PureComponent{
    ...
    handleClickProgress = () => {
        const Id = this.Id;
        router.push('/bidSection/progressMana/' + Id);
    }
    //质安管理
    handleClickQualSecu = () => {
        const Id = this.Id;
        router.push('/bidSection/QualitySecurityMana/' + Id);
    }
    //资料管理
    handleClickFile = () => {
        const Id = this.Id;
        router.push('/bidSection/fileMana/' + Id);
    }
    //资料管理
    handleClickDesign = () => {
        const Id = this.Id;
        router.push('/bidSection/DesignMana/' + Id);
    }
    componentDidMount = () =>{
        this.getData();
        this.getProps();
    ...
    render(){
        return(
            ...
            >
                ...
                <Card title='基本信息' style={{ margin: '12px 0' }}>
                    <DescriptionList>
                ...
                <Card title='人员/进度' style={{ margin: '12px 0' }}>
                ...
                <Card title='施工单位' style={{ margin: '12px 0' }}>
                    <DescriptionList>
                ...
                <Card title='子项目信息' style={{ margin: '12px 0' }}>
                    <Table
                        size='small'
                        rowKey={record => record.ID}
                        columns={this.projectCols}
                        dataSource={this.state.courtProjects}
                        title={()=> <h3><Icon type="hdd" style={{marginRight:'10px'}}/>老旧小区</h3>}
                    >
                    </Table> 
                    <Table
                        size='small'
                        rowKey={record => record.ID}
                        columns={this.projectCols}
                        dataSource={this.state.roadProjects}
                        title={()=> <h3><Icon type="block" style={{marginRight:'10px'}}/>背街小巷</h3>}
                    >
                    </Table>                      
                </Card>
            </PageHeaderWrapper>
